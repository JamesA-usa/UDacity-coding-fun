...
After cleaning and normalizing the data; the X and y variables were assigned, and the data was trained and split into X_train, X_test, y_train, and y_test with the standard test/train split of 20/80 percent and a random state of 42 assigned based on the data size and chosen model. The linear regression model was appealing based on simplicity and interpretability, though it was not attempted due the requirement to process the categorical data further and it was not ideal for features with less than eight ordinal levels. The tree classification model was desired but after analyzing the data, a lack of bimodal features was noted and using this model required additional processing. The random forest regression model executed successfully without the need for additional processing. For this survey data, the random forest model proved to be the best method out of all the methods learned so far. This method is very reliable and commonly used in machine learning.
